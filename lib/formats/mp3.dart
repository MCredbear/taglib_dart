...
class Mp3File {
  ...
  String? getGenre() {
    for (final frame in _frames) {
      if (listEquals(frame.name, 'TCON'.codeUnits)) {
        return defaultReading(frame.data);
      }
    }
    return null;
  ...
  void setGenre(String? genre) {
    if (genre != null) {
      var data = [0x03] + utf8.encode(genre) + [0x00];
      _frames
          .firstWhere(
            (frame) => listEquals(frame.name, 'TCON'.codeUnits),
            orElse: () {
              _frames.add(ID3Frame('TCON'.codeUnits, List.empty()));
              return _frames.last;
            },
          )
          .data = data;
